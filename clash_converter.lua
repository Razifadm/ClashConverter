...
local lan_ip = sys.exec("uci get network.lan.ipaddr"):match("%S+") or "192.168.1.1"
local OPENCLASH_UI = "http://" .. lan_ip .. "/cgi-bin/luci/admin/services/openclash"
...
function action_index()
    ...
    if method == "POST" then
        ...
        http.prepare_content("text/html; charset=utf-8")
        http.write('<h2>Preview generated YAML</h2>')
        for i,rec in ipairs(saved_preview) do
            ...
            http.write('<pre>' .. html_escape(rec.yaml) .. '</pre><br/>')
        end
        if #errors > 0 then
            http.write("<h3>Errors</h3><ul>")
            for _,e in ipairs(errors) do
                http.write("<li>" .. html_escape(e) .. "</li>")
            end
            http.write("</ul>")
        end
        -- Button style untuk OpenClash
        http.write(string.format([[
            <div>
                <a href="%s" style="display:inline-block;background-color:#007BFF;color:white;padding:8px 14px;border-radius:5px;text-decoration:none;">Open OpenClash UI</a>
                <a href="%s" style="margin-left:10px;">OK</a>
            </div>
        ]], html_escape(OPENCLASH_UI), html_escape(http.getenv("SCRIPT_NAME") .. http.getenv("PATH_INFO") or "/admin/services/clash_converter")))
        return
    end
    http.prepare_content("text/html; charset=utf-8")
    http.write('<h1>Clash Converter</h1>')
    http.write('<form method="POST"><textarea name="pastebox" rows="10" cols="80"></textarea><br>')
    http.write('<label><input type="checkbox" name="overwrite" value="1"> Overwrite existing file</label><br>')
    http.write('<label><input type="checkbox" name="autoreload" value="1"> Auto reload OpenClash</label><br>')
    http.write('<button type="submit">Convert & Preview</button> ')
    http.write(string.format('<a href="%s" style="display:inline-block;background-color:#007BFF;color:white;padding:8px 14px;border-radius:5px;text-decoration:none;">Open OpenClash UI</a>', html_escape(OPENCLASH_UI)))
    http.write('</form>')
end
